...
#include "Mass/Processor/TWSteeringProcessors.h"
...
//----------------------------------------------------------------------//
UTWSteerToMoveTargetProcessor::UTWSteerToMoveTargetProcessor()
	: EntityQuery(*this)
{
...
void UTWSteerToMoveTargetProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	EntityQuery.AddRequirement<FTransformFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddRequirement<FMassMoveTargetFragment>(EMassFragmentAccess::ReadWrite);
	EntityQuery.AddRequirement<FTWSteeringFragment>(EMassFragmentAccess::ReadWrite);
	EntityQuery.AddRequirement<FTWStandingSteeringFragment>(EMassFragmentAccess::ReadWrite);
	...
	EntityQuery.AddConstSharedRequirement<FTWMovingSteeringParameters>(EMassFragmentPresence::All);
	EntityQuery.AddConstSharedRequirement<FTWStandingSteeringParameters>(EMassFragmentPresence::All);
...
void UTWSteerToMoveTargetProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [this, World](FMassExecutionContext& Context)
	{
		...
		const TArrayView<FTWSteeringFragment> SteeringList = Context.GetMutableFragmentView<FTWSteeringFragment>();
		const TArrayView<FTWStandingSteeringFragment> StandingSteeringList = Context.GetMutableFragmentView<FTWStandingSteeringFragment>();
		...
		const FTWMovingSteeringParameters& MovingSteeringParams = Context.GetConstSharedFragment<FTWMovingSteeringParameters>();
		const FTWStandingSteeringParameters& StandingSteeringParams = Context.GetConstSharedFragment<FTWStandingSteeringParameters>();
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			...
			FTWSteeringFragment& Steering = SteeringList[EntityIt];
			FTWStandingSteeringFragment& StandingSteering = StandingSteeringList[EntityIt];
