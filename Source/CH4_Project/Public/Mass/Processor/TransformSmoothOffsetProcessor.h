...
#include "TransformSmoothOffsetProcessor.generated.h"
...
UCLASS()
class CH4_PROJECT_API UTransformSmoothOffsetProcessor : public UMassProcessor
{
	GENERATED_BODY()

public:
	UTransformSmoothOffsetProcessor()
	{
	...
	virtual void ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager) override
	{
		...
		EntityQuery.AddConstSharedRequirement<FTransformOffsetParams>();
	...
	virtual void Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context) override
	{
		...
		{
			const TArrayView<FTransformOffsetFragment>& TranslationOffsets = Context.GetMutableFragmentView<
				FTransformOffsetFragment>();
			const FTransformOffsetParams& Params = Context.GetConstSharedFragment<FTransformOffsetParams>();

			const float DeltaTime = Context.GetWorld()->DeltaTimeSeconds;
			for (int32 EntityIndex = 0; EntityIndex < Context.GetNumEntities(); ++EntityIndex)
			{
				FTransformOffsetFragment& TranslationOffset = TranslationOffsets[EntityIndex];
